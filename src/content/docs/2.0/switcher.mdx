...
text: Switcher
title: Switcher
lead: Dynamically transition through different content panes.
meta:
...
In this example we are using the [Subnav component](/docs/2.0/subnav).

<Snippet component="switcher.basic" />
...
<Snippet component="switcher.navigation_controls" />
...
<Snippet component="switcher.multiple_containers" />

## Animations

You can apply all animations from the [Animation component](/docs/2.0/animation) to switcher items. To do so, add the `animation` parameter with the relevant class to the `data-uk-switcher` attribute.
...
<Snippet component="switcher.animations" />
...
<Snippet component="switcher.multiple_animations" />

## Switcher and subnav

The switcher is easily applied to the [Subnav component](/docs/2.0/subnav).
...
<Snippet component="switcher.subnav" />

## Switcher and tab

As an exception to the rule, add the `data-uk-tab` attribute instead of `data-uk-switcher` to the tabbed navigation to combine the switcher with the [Tab component](/docs/2.0/tab).
...
<Snippet component="switcher.tab" />
...
<Snippet component="switcher.vertical_tabs" />

## Switcher and button

The switcher can also be applied to buttons or button groups from the [Button component](/docs/2.0/button). Just add the switcher attribute to a block around a group of buttons or to the element with the `.uk-btn-group` class.
...
<Snippet component="switcher.button" />

<span class="uk-label uk-label-primary">Note</span> Since this example does not
wrap the buttons into list items the clickable elements which trigger the
content switching has to be changed by using the `toggle` option.

## Switcher and nav

To apply the switcher to the [Nav component](/docs/2.0/nav), add the `data-uk-switcher` attribute to the nav `<ul>` element. Use the [Grid](/docs/2.0/grid) and [Width](/docs/2.0/width) components to arrange nav and content in a grid layout.
...
<Snippet component="switcher.nav" />
...
- The _toggle navigation_ has the `tablist` role, and if it is a [Nav component](/docs/2.0/nav), the `aria-orientation` state set to `vertical`.
