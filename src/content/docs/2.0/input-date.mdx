...
<span class="uk-label uk-label-destructive">Warning</span> This is a brand new component. Please use it with caution and be aware that you may encounter unexpected behavior or errors. If you experience any issues or bugs, please file an issue report so that we can investigate and improve the component.
...
To customize the display format, we use [DayJS](https://day.js.org/docs/en/parse/string-format) under the hood. You can use their parsing tokens and pass them to the `display-format` attribute.
...
{/* add icon, drop, with-time, require-time, display-format */}
