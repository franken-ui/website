...
component](/docs/2.0/card) for visualization.

<Snippet component="drop.basic" />
...
<Snippet component="drop.parent_icon" />
...
<Snippet component="drop.position" />
...
<Snippet component="drop.boundary" />
...
<Snippet component="drop.shift_flip" />
...
{/* <Snippet component="drop.stretch" index={9} /> */}

## Animation

Apply one or more animations to the dropdown by adding the `animation: uk-anmt-*` option with one of the classes from the [Animation component](/docs/2.0/animation). You can also determine the animation's duration. Just add the `duration` option with a value in milliseconds. Set `animate-out: true` to also show an animation when closing the drop.

```html
<div data-uk-drop="animation: uk-anmt-slide-top-small; duration: 1000"></div>
```

<Snippet component="drop.animation" />
...
<Snippet component="drop.animation_1" />
